...
public static class Translations
{
    ...
    private static readonly Dictionary<string, (string Es, string En)> Map = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["UpdateAvailable"] = ("Hay una versión nueva ({0}). Podés descargarla en {1}", "A new version is available ({0}). You can download it at {1}"),
        ...
        ["TemperatureHint"] = ("Controla la creatividad y aleatoriedad de las respuestas. Valores bajos son más deterministas, valores altos más variados.", "Controls the creativity and randomness of responses. Lower values are more deterministic, higher values more varied."),
        ["MaxTokens"] = ("Máx. tokens de salida", "Max output tokens"),
        ["MaxTokensHint"] = ("Tope de tokens que la IA puede usar por llamada. En AUTO se calcula el valor ideal según la cantidad de archivos del lote, y nunca supera el límite real del modelo.", "Max tokens the AI may use per call. In AUTO it picks the ideal value from the batch file count, never exceeding the model's real limit."),
        ["MaxTokensAuto"] = ("Auto", "Auto"),
        ["ModelLimitLabel"] = ("Límite del modelo: {0} tokens", "Model max output: {0} tokens"),
